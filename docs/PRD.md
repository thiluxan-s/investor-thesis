...
- **Why pgvector and not Pinecone/Weaviate?** We already have Postgres. Embeddings here are for dedup and similar-evidence lookup, not large-scale vector search. pgvector is plenty, costs nothing extra, and shows that I reach for managed complexity only when needed.
