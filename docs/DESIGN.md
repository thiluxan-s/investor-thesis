...
### Claim drill-down (Phase 7c)

**Goal:** Make every health bar in the app answerable with "why" — click a claim, see the evidence that produced its score, with enough honesty about the math that the surface teaches the health model rather than just decorating it.

**Evidence rows show both signed contribution and weight share, not contribution alone.** A `neutral` verdict contributes `0` to a claim's score, so a contribution-only row makes it read as inert. It isn't: a heavy neutral item is diluting the score toward zero, which is exactly why an otherwise strong claim reads muted. Showing weight share alongside contribution (`ClaimEvidenceList`, backed by `rankContributions` in `lib/health/contribution.ts`) is what lets a reader see that.

**Rows are ordered by raw weight, not contribution magnitude.** Sorting by `|contribution|` would bury the heavy-neutral row that most needs to be near the top to explain a muted score. Weight — confidence decayed by age, before impact is applied — is the ordering key.

**The research/challenge split is never rendered as parts of a total.** No stacked bar, no percentage-of-whole. `claimHealthBreakdown`'s `research` and `challenge` scores are two independent weighted averages over different denominators; they don't sum or average to `overall`. `HealthSplit` states this in standing copy rather than leaving it implicit: "Each score is what that line of inquiry found on its own. They are separate weighted averages, not two halves of the overall score." This is the one idea on this screen most likely to be misread if left to the visual layout alone, so it's said outright.

**A line of inquiry with zero links renders as an absence, never a neutral `0.00`.** Consistent with the existing "Not analyzed" convention (Phase 4b) — an empty subset is a missing finding, not a finding of zero. `HealthBar` gained an `emptyLabel` prop (default `"Not analyzed"`) so `HealthSplit` can say "No research runs yet" / "No challenge runs yet" instead.

**The latest challenge brief on the thesis page is a bookmark, not a second copy of the argument.** `LatestChallengeBrief` shows only the headline and a link into the run trace — the full case, with its citations, stays in one place. Duplicating the argument on the dashboard would create two surfaces that could drift, for a case that's one click away either way.

**`IMPACT_STYLE` is shared from `lib/health/display.ts`.** The run-trace evidence card and the drill-down's evidence rows previously risked drifting on their own copies of the impact-chip styling (dot color, text color, label per `strengthens`/`neutral`/`weakens`); centralizing it means the two surfaces literally cannot disagree on what a verdict looks like.

#### Design pass

1. The drill-down shipped as a centered `max-w-3xl` column while the thesis page it drills down from is a `1fr / 280px` split at `max-w-6xl` — navigating between them changed the page's geometry, and it's the centered-narrow-column anti-pattern besides. Rebuilt on the parent's grid: claim statement full width, evidence in the main column, the score summary in a **sticky** right rail. The stickiness is the actual argument for the layout — every evidence row states what it contributed, which only means something while the total it moved is still on screen.
2. **The summary comes first in the DOM, and `lg:order` moves it right on desktop.** Unlike the thesis page, whose sidebar holds secondary metadata and can safely fall below the content on mobile, the score here *is* the subject; stacking it under seven evidence rows on a phone buries the thing the page exists to explain.
3. `HealthSplit` moved from `sm:grid-cols-2` to stacked. Two columns inside a 280px rail squeeze each bar until the fill is unreadable — and stacking has a second benefit, since a left/right pair invites reading the two scores as halves of one total, which is exactly what the copy says they aren't.
4. The unanalyzed state was a bordered `zinc-50` box for two lines of copy. Made flat (hairline top rule + spacing), applying the same call made for `NoChallengeBrief` in the 7b pass: a short statement of outcome hasn't earned a border. Also gave it a bolded lead line so it has hierarchy rather than being one grey paragraph.
5. `Evidence · 7 items` → `Evidence · 7`. The section is titled "Evidence", so the counted noun only repeats the heading. `HealthSplit`'s "7 verdicts" keeps its noun because there it disambiguates *what* is being counted inside a line of inquiry — the two aren't inconsistent, they're doing different jobs.
6. **Breadcrumbs at three levels are now segmented, on the drill-down and the run trace alike.** Both previously wrapped the whole trail — `Theses / NVDA / Claim 1` — in one link pointing at the thesis, so "Theses" went anywhere except the thesis index. Each segment now points where it reads. The run trace was changed alongside because two adjacent three-level trails behaving differently is worse than either choice on its own.
7. Claim rows in `ClaimList` and `DemoClaimList` signalled clickability only by shifting colour on hover, with nothing at rest. Added a hover underline — matching the link treatment already used in `ClaimEvidenceList` and `ChallengeBrief` — rather than an always-on underline, which is too heavy under a full sentence of body text.
...
- **2026-08-21 — Claim drill-down rebuilt on the thesis page's `1fr / 280px` grid with a sticky score rail**, replacing a centered `max-w-3xl` column; the summary stays on screen because each evidence row's contribution is meaningless without the total it moved.
- **2026-08-21 — On the drill-down the sidebar leads in DOM order and `lg:order` moves it right**, because unlike the thesis page's secondary metadata, the score is the subject and must not fall below the evidence on mobile.
- **2026-08-21 — Three-level breadcrumbs segmented per-crumb on both the drill-down and the run trace.** A single link across the whole trail meant "Theses" didn't reach the thesis index.
- **2026-08-21 — Claim rows get a hover underline, not just a colour shift**, matching the app's existing link treatment; an always-on underline is too heavy under a sentence of body text.
- **2026-08-20 — Claim drill-down evidence rows show signed contribution AND weight share, ordered by raw weight.** Contribution alone makes a `neutral` verdict look inert when it's actively diluting the score; ordering by weight (not `|contribution|`) keeps the heavy-neutral row that explains a muted score near the top.
- **2026-08-20 — Research/challenge split is stated in copy as two independent averages, never rendered as parts of a total.** `HealthSplit` carries the sentence outright rather than trusting the layout to imply it, since the two sub-scores over different denominators genuinely don't sum to `overall`.
- **2026-08-20 — Empty line of inquiry renders as absence via a new `HealthBar` `emptyLabel` prop**, not a neutral `0.00` — consistent with the existing "Not analyzed" convention.
- **2026-08-20 — The thesis-page challenge brief is a bookmark (headline + link), not a duplicate of the full argument.** The complete case with citations stays in the run trace; the dashboard doesn't maintain a second copy that could drift.
- **2026-08-20 — `IMPACT_STYLE` centralized in `lib/health/display.ts`**, consumed by both the trace evidence card and the drill-down's rows, so the two surfaces cannot disagree on what a verdict looks like.
